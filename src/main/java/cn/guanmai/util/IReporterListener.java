...
public class IReporterListener implements IReporter {
	...
	private static final String FILE_NAME = "gm.html";

	private static final String manage_url = ConfigureUtil.getValueByKey("manageUrl");
	private static final String manage_username = ConfigureUtil.getValueByKey("manageName");
	private static final String manage_password = ConfigureUtil.getValueByKey("managePwd");

	private static final String station_url = ConfigureUtil.getValueByKey("stationUrl");
	private static final String station_username = ConfigureUtil.getValueByKey("stationName");
	private static final String station_password = ConfigureUtil.getValueByKey("stationPwd");

	private static final String bshop_url = ConfigureUtil.getValueByKey("bshopUrl");
	private static final String bshop_username = ConfigureUtil.getValueByKey("bshopName");
	private static final String bshop_password = ConfigureUtil.getValueByKey("bshopPwd");
	private static final String cms_key = ConfigureUtil.getValueByKey("cms_key");
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
		...
		for (ISuite suite : suites) {
			...
			if (suiteTest != null) {
				...
				TestResult.pass.addAndGet(suitePassSize);
				TestResult.fail.addAndGet(suiteFailSize);
				TestResult.skip.addAndGet(suiteSkipSize);
...
//		htmlReporter.config().setTestViewChartLocation(ChartLocation.TOP);
		...
		extent.setSystemInfo("manage_url", manage_url);
		extent.setSystemInfo("manage_username", manage_username);
		extent.setSystemInfo("manage_password", manage_password);

		extent.setSystemInfo("station_url", station_url);
		extent.setSystemInfo("station_username", station_username);
		extent.setSystemInfo("station_password", station_password);

		extent.setSystemInfo("bshop_url", bshop_url);
		extent.setSystemInfo("bshop_username", bshop_username);
		extent.setSystemInfo("bshop_password", bshop_password);
		extent.setSystemInfo("cms_key", cms_key);
		...
		if (tests.size() > 0) {
			...
			for (ITestResult result : treeSet) {
				...
				while (logIterator.hasNext()) {
					...
					if (details != null && details.contains(ReportUtil.getSpiltTimeAndMsg())) {
