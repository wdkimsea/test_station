...
/* 
* @author liming 
* @date Nov 6, 2018 9:51:51 AM 
* @des 品类测试类
* @version 1.0 
*/
public class PinleiCreateTest extends LoginStation {
	 ...
	 */
	@Test
	public void pinleiCreateTestCase01() {
		try {
			Reporter.log("新建品类分类");
			String name = StringUtil.getRandomString(6);
			PinleiBean pinlei = new PinleiBean(category2_id, name);

			pinlei_id = categoryService.createPinlei(pinlei);
			Assert.assertNotEquals(pinlei_id, null, "新建品类失败");
		} catch (Exception e) {
			logger.error("新建品类分类遇到错误: ", e);
			Assert.fail("新建品类分类遇到错误: ", e);
		}

	}

	/**
	 * 新建二级分类,使用已经存在的二级分类名称
	 * 
	 */
	@Test
	public void pinleiCreateTestCase02() {
		try {
			Reporter.log("新建品类分类,使用已经存在的品类分类名称");
			String name = StringUtil.getRandomString(6);
			PinleiBean pinlei = new PinleiBean(category2_id, name);
			...
			pinlei_id = categoryService.createPinlei(pinlei);
			Assert.assertEquals(pinlei_id, null, "新建二级分类,名称使用已经存在的二级分类名称,断言失败");
		} catch (Exception e) {
			logger.error("新建品类分类遇到错误: ", e);
			Assert.fail("新建品类分类遇到错误: ", e);
		}
	 ...
	 */
	@Test
	public void pinleiCreateTestCase03() {
		try {
			Reporter.log("新建品类分类,使用删除的品类分类名称");
			String name = StringUtil.getRandomString(6);
			PinleiBean pinlei = new PinleiBean(category2_id, name);

			String tmp_pinlei_id = categoryService.createPinlei(pinlei);
			Assert.assertNotEquals(tmp_pinlei_id, null, "新建品类失败");
			...
			pinlei_id = categoryService.createPinlei(pinlei);
			Assert.assertNotEquals(pinlei_id, null, "新建品类分类,名称使用删除的品类分类名称,断言成功");
		} catch (Exception e) {
			logger.error("新建品类分类遇到错误: ", e);
			Assert.fail("新建品类分类遇到错误: ", e);
		}
	...
	public void afterMethod() {
		try {
			...
			logger.error("删除品类分类遇到错误: ", e);
			Assert.fail("删除品类分类遇到错误: ", e);
		}
